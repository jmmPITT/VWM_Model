...
import torch as T
from network_sensor_test import *
...
if __name__ == '__main__':
    ...
    encoder = load_model('tmp/vae_model.pth').to(device)
    encoder.eval()
    n_games = 100
    ...
    linear_spaced_theta = np.linspace(0.0, 30.0, 15)
    reward_list = np.zeros((15,100,1))
    for k in range(15):
        cue_array = 0
        target_array = 0
        A_list = []
        for i in range(n_games):
            ...
            env.proportion = 1.0
            # env.change_time = 25
            env.orientation_change = linear_spaced_theta[k]
            env.change_true = 1
            env.change_index = 0
            ...
            while not done:
                ...
                transformer_state, A = Transformer(state, env.t, m)
                # print("A",A.shape)
    ...
    np.save("hits_cueValid100",reward_list)