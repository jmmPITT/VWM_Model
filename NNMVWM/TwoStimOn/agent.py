...
class Agent:
    ...
    def __init__(
        ...
        self.actor = ActorNetwork(alpha, input_dims=input_dims,
                                  ...
                                  n_actions=n_actions,
                                  name='actor_planner'+name, chkpt_dir='td3_MAT'
                                  ).to(self.device)
    ...
    def learn(self, Transformer):
        ...
        transformer_state, _ = Transformer(state, mask)

        # Compute target distribution
        with T.no_grad():
            # Next-state transformer
            transformer_state_, _ = Transformer(state_, mask_)
            target_actions = self.target_actor.forward(transformer_state_)
            q1_ = self.target_critic_1.forward(transformer_state_, target_actions)

        # Evaluate current Q distribution for the chosen actions
        q1 = self.critic_1.forward(transformer_state, action)
        ...
        with T.no_grad():
            ...
            Q0_ = self.target_critic_1.forward(transformer_state, a0)
            Q1_ = self.target_critic_1.forward(transformer_state, a1)
            ...
            p = self.target_actor.forward(transformer_state)
            pa0_ = p[:, 0]
            pa1_ = p[:, 1]
        ...
        action_probs = self.actor.forward(transformer_state)
        pa0 = action_probs[:, 0]
        pa1 = action_probs[:, 1]
