...
if __name__ == '__main__':
    for modNUM in range(20):
        # --------------------------------------------------------------------------
        # Basic setup
        # --------------------------------------------------------------------------
        env = 0  # Placeholder; set to 0 before re-initializing with the environment
        T_end = 8  # Maximum length of each trial
        input_dim = (128 + 4 + 8) * 4  # Dimensions for the agent's input
        ...
        agent.load_models()
        Transformer.load_state_dict(T.load("transformer10_td3"))
