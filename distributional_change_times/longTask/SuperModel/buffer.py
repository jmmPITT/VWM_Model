...
class ReplayBuffer():
    def __init__(self, max_size, input_shape, n_actions, alpha=0.7):
        ...
        self.state_memory = np.zeros((self.mem_size, input_shape*31))
        self.mask_memory = np.zeros((self.mem_size, 1*31*4))

        self.new_state_memory = np.zeros((self.mem_size, input_shape*31))
        self.next_mask_memory = np.zeros((self.mem_size, 1*31*4))
